...
export default function AdminProducts() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-products'] }); toast.success(editing ? 'Product updated' : 'Product created'); closeModal(); },
  ...
  const openEdit = (p: Product) => { setEditing(p); setForm({ name: p.name, description: p.description, price: p.price, stock: p.stock, category: p.category, imageUrl: p.imageUrl, active: p.active }); setModal(true); };
            ...
            <tbody className="divide-y divide-gray-50">
              {products.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-semibold text-gray-900">{formatNGN(p.price)}</td>
                  <td className="px-4 py-3">
